...
int CopyMuxWithProtobufMeta() {
	...
	// 2. fflags + genpts: Automatically generate PTS timestamps
	av_dict_set(&mux_dict, "fflags", "+genpts", AV_DICT_APPEND);

    // open input file
    ret = avformat_open_input(&in_fmt, g_srcProtoPath.c_str(), nullptr, nullptr);
    if (ret < 0) goto cleanup;
    ret = avformat_find_stream_info(in_fmt, nullptr);
    if (ret < 0) goto cleanup;

    // Create output MP4 container
    ret = avformat_alloc_output_context2(&out_fmt, nullptr, "mp4", g_dstProtoPath.c_str());
    if (ret < 0) goto cleanup;
    ...
    pbUsr.set_name("linmingyang");
	pbUsr.set_company("OBSBOT");
	pbUsr.set_type("protobuf");

	pbUsr.SerializeToString(&pbStr);

    av_dict_set(&out_fmt->metadata, "video_custom_pb_data", pbStr.c_str(), 0);

    // open outPut file IO
    if (!(out_fmt->oformat->flags & AVFMT_NOFILE))
    {
        ret = avio_open(&out_fmt->pb, g_dstProtoPath.c_str(), AVIO_FLAG_WRITE);
        if (ret < 0) goto cleanup;
    }
...
int VerifyProtobufMeta() {
    ...
    AVDictionaryEntry* entry_pb = av_dict_get(fmt_ctx->metadata, "video_custom_pb_data", nullptr, 0);

    if (!entry_pb)
    {
        std::cerr << "don't find Protobuf metadata" << std::endl;
        avformat_close_input(&fmt_ctx);
        return -1;
    }

    const char* b64_str = entry_pb->value;

    Usr result;
    bool parse_ok = result.ParseFromString(b64_str);

    std::cout << "\n==================== Check Result ====================\n";
    if (parse_ok)
    {
        result.PrintDebugString();
        std::cout << "Protobuf data read/write success\n";
    }
    else
    {
        std::cerr << "Protobuf parse fail\n";
        ret = -1;
    }
