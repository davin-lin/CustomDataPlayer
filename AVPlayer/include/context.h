...
private:
    ...
    AVFormatContext* fmtCtx_ = nullptr; // 解封装器上下文

    int             audioIndex_ = -1;          // 音频流索引
    AVCodecContext* audioCodecCtx_ = nullptr; // 音频流解码器上下文
    AVStream* audioStream_ = nullptr;    // 音频流
    PacketQueue     audioPacketQueue_;        // 音频packet队列
    ...
    AVCodecContext* videoCodecCtx_ = nullptr; // 视频流解码器上下文
    AVStream* videoStream_ = nullptr;    // 视频流
    PacketQueue     videoPacketQueue_;        // 视频packet队列
    ...
    AVCodecContext* subtitleCodecCtx_ = nullptr; // 字幕流解码器上下文
    AVStream* subtitleStream_ = nullptr;    // 字幕流
    PacketQueue     subtitlePacketQueue_;        // 字幕packet队列
    ...
    // seek操作
    std::atomic<bool> seekReq_ = false;    // seek操作
    int seekFlags_ = 0;
    int64_t seekPos_ = 0;
    int64_t seekRel_ = 0;
